        ...
        function updateChart(grades) {
            ...
            window.gradeChart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: grades.map(g => g.code || g.name),
                    datasets: [{
                        label: 'Grade Points',
                        data: grades.map(g => Math.max(0, g.grade)),
                        backgroundColor: grades.map(g => g.grade > 0 ? '#2563eb' : '#ef4444'),
                        borderColor:     grades.map(g => g.grade > 0 ? '#1d4ed8' : '#b91c1c'),
                        borderWidth: 1,
                        borderRadius: 4
                    }]
                },
...
function exportPDF() {
            ...
            doc.text('JCT CET CGPA Calculator — B.E. Electrical and Electronics Engineering', pageWidth/2, 18, { align: 'center' });
            ...
            if (lastCalcData.mode === 'gpa') {
                ...
                if (lastCalcData.hasArrears) {
                    ...
                    doc.setFontSize(8);
                    doc.setFont('helvetica', 'normal');
                    doc.text('⚠ Subjects graded U / W / SA are excluded from CGPA denominator (JCT CET CGPA Calculator Regulations).', 14, y + 27);
                }
            }
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(`Generated on ${ts} | JCT CET CGPA Calculator(R21)`, pageWidth/2, doc.internal.pageSize.getHeight() - 10, { align: 'center' });
            }
        ...
        function buildResultCardHTML(data) {
            ...
            const header = `
                <div class="rc-header">
                    <div class="rc-title">JCT CET CGPA Calculator — B.E. Electrical and Electronics Engineering</div>
                    <div class="rc-subtitle">GPA / CGPA Academic Record</div>
                </div>
                <hr class="rc-divider">`;

            const footer = `<div class="rc-footer">Generated on ${now} &nbsp;|&nbsp; JCT CET CGPA Calculator EEE GPA Calculator</div>`;
            ...
            if (data.mode === 'gpa') {
                ...
                const arrearNote = data.hasArrears
                    ? `<p style="font-size:11px;color:#b91c1c;margin:6px 0 0;">⚠ Subjects graded U / W / SA are excluded from CGPA denominator (JCT CET CGPA Calculator Regulations).</p>`
                    : '';
